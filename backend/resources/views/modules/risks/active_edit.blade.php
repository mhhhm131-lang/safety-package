...
@section('page_title', 'تعديل الخطر الفعلي')
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1" style="color:var(--text-main);">
                <i class="bi bi-pencil-square me-2" style="color:#e74c3c;"></i>تعديل الخطر الفعلي
            </h4>
        ...
        <a href="{{ route('risk.active.index') }}" class="btn btn-outline-secondary"><i class="bi bi-arrow-right me-1"></i> رجوع</a>
    </div>
    ...
    <form method="POST" action="{{ route('risk.active.update', $risk) }}">
        @csrf
            ...
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-4" data-unit-wrap hidden>
                        ...
                        @include('governance.places._unit_select', ['units' => $units, 'placeSelect' => 'place_id', 'selected' => old('place_unit_id', $risk->place_unit_id)])
                    </div>
                </div>
            </div>
        </div>

        {{-- 4. التعيينات (خاص بالفعلي) --}}
        <div class="card mb-3" style="background:var(--bg-card);border:1px solid var(--border-color);">
            <div class="card-header" style="background:transparent;border-bottom:1px solid var(--border-color);">
                <h6 class="mb-0 fw-bold" style="color:var(--accent);"><i class="bi bi-person-check me-2"></i>التعيينات والمتابعة</h6>
            </div>
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="date" name="target_closure_date" class="form-control"
                               value="{{ old('target_closure_date', $risk->target_closure_date?->format('Y-m-d')) }}">
                    </div>
                    <div class="col-md-8">
                        <label class="form-label" style="color:var(--text-main);">ملاحظات</label>
                        <textarea name="notes" class="form-control" rows="2">{{ old('notes', $risk->notes) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
                                ...
                                @foreach(\App\Modules\Risk\Models\AffectedGroup::CIRCLES + ['أخرى' => []] as $circle => $circleNames)
                                    ...
                                    <div class="d-flex align-items-center gap-2 p-2 mb-2 rounded" style="background:var(--bg-main);">
                                        <input class="form-check-input" type="checkbox"
                                               ...
                                               value="{{ $gid }}" @checked($isChecked) id="aag_{{ $phaseKey }}_{{ $gid }}">
        ...
        <div class="d-flex gap-2">
            <button type="submit" class="btn btn-danger btn-lg px-5"><i class="bi bi-check-lg me-2"></i>تحديث الخطر الفعلي</button>
            <a href="{{ route('risk.active.index') }}" class="btn btn-outline-secondary btn-lg px-4">إلغاء</a>
        </div>
    </form>
</div>
...
// Load initial cascading values on page load
